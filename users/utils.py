...
def get_users_viewable_reviews(user):
    ...
    # https://docs.djangoproject.com/fr/4.2/ref/models/querysets/
    #  le __in est dedans cherche __in
    ...
    blocked_user = UserBlock.objects.filter(user=user).values_list(
    ...
    # je recup les reviews concernant mon ticket pas dans
    # les user que je follow et moi meme
    # si je bloque un user je verrai pas sa réponse a mon ticket,
    # mais si lui me bloque je verrai quand meme sa réponse a mon ticket
    # je laisse comme sa pour pas buguer car sinon sa fairai un
    # ticket deja répondu mais on pourrait rien voir donc un peu bizarre
    no_viewable_reviews = (
        Review.objects.filter(ticket__in=user_tickets)
        .exclude(user__in=followed_users)
        .exclude(user__in=blocked_user)
    )
