...
def abonnement_page(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            blocked = UserBlock.objects.filter(user=request.user, blocked_user=user_to_follow)
            blocked_back = UserBlock.objects.filter(user=user_to_follow, blocked_user=request.user)
            if blocked.exists() or blocked_back.exists():
                message = "Impossible de suivre un utilisateur bloqué / qui vous a bloqué"
                return render(request, 'users/abonnement.html', {'form': form, "formblock": formblock,'message': message, 'followers':followers, 'followed_users':followed_users, "blocked": blocked_user})
